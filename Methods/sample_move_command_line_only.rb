...
def move_samples(sample_names,study_to_move_samples_from,study_to_move_samples_to,user_login,rt_ticket,mode)
  ...
  ActiveRecord::Base.transaction do
    ...
    sample_names.each do |sample_name|
      ...
      sample.aliquots.where(study_id: study_to_move_samples_from).find_each do |aliquot|
        ...
        aliquot.receptacle.tap do |asset|
          ...
          requests = asset.requests.where(initial_study_id: study_to_move_samples_from)
          requests.map(&comment_on)
          requests.map(&:submission).compact.map(&submissions.method(:<<))

          asset.asset_groups.where(study_id: study_to_move_samples_from).find_each do |asset_group|
      ...
      sample.study_samples.find_each do |study_sample|
        begin
          ...
          study_links = StudySample.where(sample_id: study_sample.sample_id, study_id: study_to_move_samples_from)
    ...
    puts "Updating all of the submissions (#{c}) encountered as part of the sample moves ..."
    ...
    sub_ids.each do |sub_id|
      ...
      if initial_requests.empty?
        ...
        submission.orders.each do |order|
          puts "\t\tOrder #{order.id} from study #{order.study_id}: #{order.assets.reload.map(&:id).inspect}"
        end
      end
      c -=1
    end   
    raise "Hell!!!... test mode" unless mode == 'run'
    if Integer === sample_names.first
      ids = Sample.where(id: sample_names).map(&:assets).flatten.select {|a| a.is_a?(Lane)}.map(&:creation_batches).flatten.uniq.map(&:id)
    else
      ids = Sample.where(name: sample_names).map(&:assets).flatten.select {|a| a.is_a?(Lane)}.map(&:creation_batches).flatten.uniq.map(&:id)
    end
    puts "batch_ids = #{ids.inspect}"
    Batch.where(id: ids).each do |batch_id|
      m=Messenger.where(target_type: "Batch", target_id: batch_id).first
      if m.nil?
        puts "Unable to find Message for #{batch_id.inspect}"
      else
        m.resend
      end
    end
    puts "Updating\n#{fluidigm_plates.inspect}" unless fluidigm_plates.empty?
    update_fluidigm_plates(fluidigm_plates.uniq) unless fluidigm_plates.empty?
  end
end

# move_samples(sample_names,study_to_move_samples_from,study_to_move_samples_to,user_login,rt_ticket,mode)
